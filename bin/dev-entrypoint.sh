...
then
  ...
  # 8: Check if the database exists, or setup the database if it doesn't, as
  # it's the case when the project runs for the first time.
  #
  ...
  if [ "$1" != "webpack-dev-server" ]; then checkdb || rails db:setup; fi
